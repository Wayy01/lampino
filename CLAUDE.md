...
**Product checkout never creates an `Order`.** The cart hands off to WhatsApp (`wa.me` deep link built in `components/site/order-fields.tsx`). The `Order` table is admin/seed-side only. The cart itself lives in `localStorage`, read through `useSyncExternalStore` in `lib/cart/provider.tsx`; the root layout passes the full catalog in so stored ids can rehydrate with live prices.
